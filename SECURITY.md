...
A mobile-style digital art passport for collecting stamps at art experiences. Ships a browser front end and an Express server under .
...
The `server/` directory contains an Express application with JWT handling, rate limiting, and server-side stamp validation. The browser front end **no longer calls it** — authentication was removed from `src/App.jsx`. Treat `server/` as unmaintained until that is resolved; reports against it are welcome but it is not currently deployed.
